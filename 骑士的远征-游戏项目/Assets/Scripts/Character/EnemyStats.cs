...
public class EnemyStats : CharacterStats
{
    ...
    public void TakeDamage(WeaponData_SO weaponData)
    {
        ...
        if(CurrentHealth<=0)                 //判断一下是否死亡，若死亡则播放死亡动画、停止运动并销毁
        {
            ...
            if (isElite == true && GameManager.Instance.enemiesTwo.Contains(gameObject))
            {
                debutEffect.SetActive(false);
                GameManager.Instance.enemiesTwo.Remove(gameObject);
            }
            Destroy(gameObject, 2f);
            GameObject instance = (GameObject)Instantiate(coin, transform.position, transform.rotation);
        }
    }
    ...
    void SwitchState()           //简单的状态机，实时改变状态，这个还要改，根据人物是否进入房间来改
    {
        ...
        else if(!isin || Vector3.Distance(player.transform.position, transform.position) > 2 * AttackRange)
        {
            enemyStates = EnemyStates.GUARD;
        }
        else
